                ...
                <form action="{{ route('admin.cards.update', $card) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="destination_type" id="destination_type" class="form-select" required>
                            <option value="url" {{ old('destination_type', $card->destination_type ?? 'url') === 'url' ? 'selected' : '' }}>URL</option>
                            <option value="folder" {{ old('destination_type', $card->destination_type ?? 'url') === 'folder' ? 'selected' : '' }}>Folder</option>
                        </select>
                    ...
                    <div class="mb-3" id="link_url_group">
                        ...
                        <input type="url" name="link_url" id="link_url" class="form-control" value="{{ old('link_url', $card->link_url) }}">
                        @error('link_url')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3" id="parent_folder_group">
                        ...
                        <select name="parent_id" id="parent_id" class="form-select">
                            ...
                            @foreach ($folderOptions as $folder)
                                <option value="{{ $folder->id }}" {{ (string) old('parent_id', $card->parent_id) === (string) $folder->id ? 'selected' : '' }}>
                                    {{ $folder->name }}
                                </option>
                            @endforeach
                        </select>
